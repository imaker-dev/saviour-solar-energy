        ...
        {FEATURES.map(({ icon: Icon, title, copy }) => (
          <div key={title} className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="relative flex h-12 w-12 shrink-0 items-center justify-center sm:h-14 sm:w-14 lg:h-16 lg:w-16">
              <div className="absolute bottom-1 right-1 h-5 w-5 rounded-full bg-primary-500 sm:h-6 sm:w-6 lg:h-8 lg:w-8" />

              <Icon
                className="relative z-10 h-8 w-8 text-white sm:h-10 sm:w-10 lg:h-12 lg:w-12"
                strokeWidth={1.8}
              />
            </div>
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-lg font-semibold leading-tight text-white sm:text-xl lg:text-2xl lg:leading-none">
                {title}
              </h3>

              <p className="mt-1 text-sm leading-relaxed text-white/70 sm:text-[15px] lg:text-lg">
                {copy}
              </p>
            </div>
          </div>
